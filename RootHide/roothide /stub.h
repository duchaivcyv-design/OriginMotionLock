...
#pragma message("roothide stub functions loaded for rootless environment...")
...
// Stub functions cho rootless

#ifdef __cplusplus
extern "C" {
#endif

static const char* rootfs_alloc(const char* path) { return path ? strdup(path) : NULL; }
static const char* jbroot_alloc(const char* path) { return path ? strdup(path) : NULL; }
static const char* jbrootat_alloc(int fd, const char* path) { return path ? strdup(path) : NULL; }
